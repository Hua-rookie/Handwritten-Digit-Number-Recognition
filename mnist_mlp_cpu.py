...
class MNIST_MLP(object):
    def __init__(self, batch_size=100, input_size=784, hidden1=32, hidden2=16, hidden3=16, hidden4=16,
                 out_classes=10, lr=0.03, max_epoch=1, print_iter=100):
    ...
    def train(self):
        ...
        for idx_epoch in range(self.max_epoch):
            ...
            #每个 epoch 在测试集上评价一下准确率
            self.evaluate('train')
            self.evaluate('test')

    def evaluate(self, type='test'):
        data = self.test_data if type=='test' else self.train_data
        pred_results = np.zeros([data.shape[0]])
        for idx in range(data.shape[0]/self.batch_size):
            batch_images = data[idx*self.batch_size:(idx+1)*self.batch_size, :-1]
            #start = time.time()
            prob = self.forward(batch_images)
            #end = time.time()
        ...
        if data.shape[0] % self.batch_size >0: 
            last_batch = data.shape[0]/self.batch_size*self.batch_size
            batch_images = data[-last_batch:, :-1]
            prob = self.forward(batch_images)
            pred_labels = np.argmax(prob, axis=1)
            pred_results[-last_batch:] = pred_labels

        accuracy = np.mean(pred_results == data[:,-1])
        print('Accuracy in %s set: %5f' %(type, accuracy))
...
def build_mnist_mlp(param_dir='weight.npy'):
    h1, h2, h3, h4, e = 128, 64, 64, 16, 30
    mlp = MNIST_MLP(hidden1=h1, hidden2=h2, hidden3=h3, max_epoch=e)
    mlp.load_data()
    mlp.build_model()
    mlp.init_model()
    mlp.train()
    mlp.save_model('mlp-%d-%d-%d-%d-%depoch.npy' % (h1, h2, h3, h4, e))
